...
{
	FILE* pf = fopen("test.txt", "w");
	fputs("abcdef", pf);  //先把代码放在输出缓冲区
	printf("睡眠10s -- 已经开始写数据了，打开test.txt文件，发现文件没有内容");
	Sleep(1000);
	printf("刷新缓冲区");
	fflush(pf);// 刷新缓冲区时，才能将输出缓冲区的数据写到文件（磁盘）
	// fflush 在高版本的vs上用不了
	printf("在睡眠10s-- 此时，再次打开test.txt文件，发现文件没有内容了");
	Sleep(10000);
	fclose(pf);
